...
from roles.helpers import login_required_ajax, user_has_insti_privilege, forbidden_no_privileges
    ...
    def create(self, request):
        if user_has_insti_privilege(request.user.profile, 'RoleB'):
            ...
            if serializer.is_valid():
                ...
                if banned_user:
                    banned_user_model = UserProfile.objects.filter(ldap_id = banned_user).first()
                    if not banned_user_model:
                        return Response({'user': ['This field is Invalid.']}, status=status.HTTP_400_BAD_REQUEST)
                
                if not duration_of_ban:
                    return Response({'duration_of_ban': ['This field is required.']}, status=status.HTTP_400_BAD_REQUEST)
