...
def new_comment(instance, created, **kwargs):  # pylint: disable=W0613
    """Notify users when new comment is written."""
    if created:
        reciever = instance.complaint.created_by.user
        notify.send(instance, recipient=reciever,
                    verb=instance.commented_by.name + " added a comment to the complaint" + instance.complaint.description)
