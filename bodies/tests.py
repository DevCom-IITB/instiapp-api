...
class BodyTestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.insti_role = InstituteRole.objects.create(
            name="TestInstiRole", permissions=["AddB"]
        )
        self.user.profile.institute_roles.add(self.insti_role)

        url = "/api/bodies"
        data = {"name": "TestBody1", "image_url": "http://example.com/image.png"}
        ...
        data = {"name": "TestBody2", "image_url": "http://example.com/image2.png"}
        response = self.client.post(url, data, format="json")
        self.assertEqual(response.status_code, 201)
        response.close()
    ...
    def test_body_update(self):
        ...
        data = {"name": "TestBody1Upd", "image_url": "http://example.com/image_new.png"}
