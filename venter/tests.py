...
class VenterTestCase(APITestCase):
    ...
    def test_model_venter(self):
        ...
        complaint = Complaint.objects.create(
            created_by=self.user.profile, description="test"
        )

        self.assertEqual(str(complaint), "test")

        tag = ComplaintTag.objects.create(tag_uri="test_tag")

        self.assertEqual(str(tag), "test_tag")

        comment = ComplaintComment.objects.create(
            complaint=complaint, text="test_comment", commented_by=self.user.profile
        )

        self.assertEqual(str(comment), "test_comment")

        complaintMedia = ComplaintImage.objects.create(
            complaint=complaint, image_url="www.google.com"
        )

        self.assertEqual(str(complaintMedia), "www.google.com")

        authority = ComplaintAuthority.objects.create(
            name="dummyauth", email="dummyauth@example.com"
        )
    ...
    def test_admin_actions(self):
        ...
        self.assertEqual(
            Complaint.objects.get(status=STATUS_RESOLVED).status, STATUS_RESOLVED
        )
        ...
        self.assertEqual(
            Complaint.objects.get(status=STATUS_IN_PROGRESS).status, STATUS_IN_PROGRESS
        )
        ...
        self.assertEqual(
            Complaint.objects.get(status=STATUS_DELETED).status, STATUS_DELETED
        )
    ...
    def test_send_mass_mail(self):
        ...
        auth_mail_1 = ComplaintAuthority.objects.create(
            email="receiver1@example.com", name="receiver1"
        )
        auth_mail_2 = ComplaintAuthority.objects.create(
            email="receiver2@example.com", name="receiver2"
        )
        ...
        complaint_single = Complaint.objects.create(
            created_by=self.user.profile, status=STATUS_IN_PROGRESS
        )
        complaint_single.authorities.add(auth_mail_1)
        image = []
        image.append(
            ComplaintImage.objects.create(
                image_url="https://www.google.com/", complaint=complaint_single
            )
        )
        ...
        Complaint.objects.create(created_by=self.user.profile, status=STATUS_REPORTED)
        authority_list = set(
            complaint_multi.authorities.values_list("email", flat=True)
        )
        ...
        new_auth3 = ComplaintAuthority.objects.create(
            email="auth3@example.com", name="auth3"
        )
        ...
        new_auth2 = ComplaintAuthority.objects.create(
            email="auth2@example.com", name="auth2"
        )
