...
class EventTestCase(TransactionTestCase):
    ...
    def test_event_prioritizer(self):  # pylint: disable=R0914,R0915
        ...
        def assertWeightOrder(events, url="/api/events"):
            ...
            for event in events:
                response_event = next(
                    x for x in response.data["data"] if x["id"] == str(event.id)
                )
    ...
    def test_event_get(self):
        ...
        event = Event.objects.create(
            name="Test #Event 123!", start_time=timezone.now(), end_time=timezone.now()
        )
    ...
    def test_event_creation(self):
        ...
        self.assertEqual(
            test_event.bodies.get(id=self.test_body_1.id), self.test_body_1
        )
    ...
    def test_event_update(self):
        ...
        response = self.client.put(
            self.update_url, self.update_event_data, format="json"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_event_update3(self):
        ...
        self.update_event_data["bodies_id"] = [
            str(self.test_body_1.id),
            str(self.test_body_2.id),
        ]
        response = self.client.put(
            self.update_url, self.update_event_data, format="json"
        )
    ...
    def test_event_update8(self):
        ...
        BodyChildRelation.objects.create(
            parent=self.test_body_2, child=self.test_body_1
        )
    ...
    def test_event_deletion(self):
        ...
        now = timezone.now()
        event = Event.objects.create(name="TestEvent", start_time=now, end_time=now)
        self.test_body_1.events.add(event)
    ...
    def test_nobody(self):
        ...
        response = self.client.post(
            "/api/events", self.update_event_data, format="json"
        )
        self.assertEqual(response.status_code, 403)
