...
url_mapping = {Event: "/event/", Body: "/org/"}
...
def explore(request):
    ...
    rendered = render_to_string(
        "explore.html", {"bodies": bodies, "settings": settings}
    )
    return HttpResponse(rendered)
...
def user_details(request, pk):
    ...
    rendered = render_to_string(
        ...
        {
            "profile": profile,
            "achievements": profile.achievements.prefetch_related(
                "body", "event"
            ).filter(hidden=False),
            "settings": settings,
        },
...
def mstile(request):
    ...
    images = (
        Event.objects.filter(end_time__gte=timezone.now())
        .order_by("start_time")
