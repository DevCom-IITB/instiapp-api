...
class CommunityTestCase(TransactionTestCase):
    ...
    def test_communitypost_alllist(self):
        ...
        self.assertListEqual(list(map(lambda x: (x['status'], x['deleted'], x['thread_rank']), data)),
    ...
    def test_communitypost_yourlist(self):
        ...
        data = response.data['data']
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['count'], CommunityPost.objects.filter(
            thread_rank=1, posted_by=self.user1.profile).count())
        self.assertListEqual(list(map(lambda x: x['posted_by']['name'], data)),
    ...
    def test_communitypost_pendinglist(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['count'], CommunityPost.objects.filter(
            thread_rank=1, status=0).count())
        self.assertListEqual(list(map(lambda x: (x['status'], x['thread_rank'], x['deleted']), data)),
    ...
    def test_communitypost_get(self):
        ...
        communitypost = CommunityPost.objects.create(
            content="Test #CommunityPost 123!", community=self.test_community_1, posted_by=self.user1.profile)
