...
def handle_entry(entry):
    ...
    for p in coordinates:
        loc =  Location.objects.get_or_create(pixel_x= p[0], pixel_y= p[1], name="Node"+i)
        loc_list.append(loc)
        i+=1

    for i in range(0, len(coordinates)):
        loc1 = loc_list[i]
        for loc2_ind, dist in adj_list[i]:
            loc2 = loc_list[loc2_ind]
            lld = LocationLocationDistance.objects.filter(location1=loc1, location2=loc2).first()
            if not lld:
